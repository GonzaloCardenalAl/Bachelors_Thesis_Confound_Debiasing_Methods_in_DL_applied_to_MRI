...
from sklearn.metrics import balanced_accuracy_score, explained_variance_score, mean_squared_error
...
def cnn_pipeline(
    ...
    X_name='X', y_name='y', cf_name=None,
    ...
    output_dir="results/", run_id='run0', save_figures=True,
...
randomly sampled n={}/{} as validation data".format(len(data_val_idx), len(y_data)))
            ...
            # In DEBUG mode, use only a small subset of the data stratified
                ...
                cf_data = None
                if cf_name is not None:
                    cf_data =data[cf_name][:debug_size]
...
{criterion.__name__} loss function doesn't have any 'weight' argument.")
                        ...
                        if earlystop_patience:
                            callbacks.extend([
                                EarlyStopping(earlystop_patience, window=2,
                                              ignore_before=earlystop_patience, 
                                              retain_metric= balanced_accuracy_score, mode='max')])
                        """
                        ...
                        if save_model:
                            ...
                            callbacks.extend([
                                ModelCheckpoint(path=output_dir, prepend=run_id, 
                                                ...
                                                retain_metric=main_metric, mode = "max")])  
                        ...
                        train_data = arrayDataset(X_data[mask], y_data[mask], cf_data[mask] if cf_data is not None else None, transform=transform, cf_name = cf_name, lbl_type=task_type)
                        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, drop_last=True) #don't use num_workers as it doesnt go well with the outer parallel jobs, multiprocessing_context='fork') #https://github.com/pytorch/pytorch/issues/44687
                        ...
                        val_data = arrayDatasetWithSubID(X_data[~mask], y_data[~mask],
                                                         i_data[~mask], cf_data[~mask] if cf_data is not None else None, transform=transform, cf_name = cf_name , lbl_type=task_type)
                        val_loader = DataLoader(val_data, batch_size=batch_size_val, shuffle=False, drop_last=True)
                        ...
                        if debug:
                            ...
                            save_fig_path = f"{output_dir}/{run_id}_sample_img" if save_figures else ''
                            ...
                            save_fig_path = f"{output_dir}/{run_id}_sample_img_aug" if save_figures else ''
                            show_MRI_stats(X_i, save_fig_path=save_fig_path,
                                title=f"Sample i={rand_sub_idx} after augmentation: X shape={X_i.shape},  y={y_i}")
                       
                        trainer = Trainer(model, criterion_fn, optimizer_fn, scheduler=scheduler_fn,
                                    metrics=metrics,  callbacks=callbacks,
                                    device=gpu, task_type=task_type, lr_beta=lr_beta, training_time_callback=None )
                        ...
                        if show_grad_flow: 
                            if save_figures: plt.savefig(f"{output_dir}/{run_id}_grad_flow.jpg", bbox_inches='tight', pad_inches=0.01)
                            plt.show()
                        
                        save_fig_path = f"{output_dir}/{run_id}_" # save always irrespective of 'save_figures' 
                        ...
                        model.load_state_dict(torch.load(str(glob(str(output_dir+ "/" + run_id + "_model-*")))[2:-2]))
                        trainer.model = model
                        
                        
                        (model_outputs, true_labels, 
                         val_report, data_other) = trainer.evaluate_model(val_loader, confounder_key=cf_name,
                                                                                     metrics=metrics,
                                                                                     return_results=True,
                        ...
                        if data_hold:
                            ...
                            cf_data_hold = None
                            if cf_name is not None:
                                cf_data_hold =data_hold[cf_name][:]
                            try:
                                data_hold.close()
                            except:
                                pass
                            ...
                            holdout_data = arrayDatasetWithSubID(X_data_hold, y_data_hold, 
                                                                 i_data_hold, cf_data_hold if cf_data_hold is not None else None, transform=transform,
                                                                 lbl_type=task_type, cf_name = cf_name)
                            holdout_loader = DataLoader(holdout_data, batch_size=batch_size_val, 
                                                        shuffle=False, drop_last=True)
                            
                            (model_outputs, true_labels, 
                             hold_report, data_other) = trainer.evaluate_model(holdout_loader, confounder_key=cf_name, 
                                                                     metrics=metrics, 
                                                                     return_results=True,
                                                                     write_to_dir='')#f"{output_dir}/{run_id}_"
