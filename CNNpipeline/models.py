...
class FCN_3D_hooked(FCN_3D):
    ...
    def __init__(self, convs, **kwargs):
        ...
        self.n=3
        self.features_3D = nn.Conv3d(convs[-1], self.n, kernel_size=self._conv_out_shape)
        self.finalconv = nn.Conv3d(self.n, self.out_classes, kernel_size=1)
...
################
###Note this ResNet only works for data with size 96,114,96 due to the conv at the end with fixed kernel size
class ResNet50DeepRepViz(nn.Module):  
    def __init__(self, out_classes=1, 
                 task_type='classif', 
                 freeze_feature_extractor=False, 
                 pretrained_model='', 
        ...
        """Args::
            ...
            debug_print(optional): If True, It will print all the layers name and weigths freeze.
        """
        super(ResNet50DeepRepViz, self).__init__()
...
class PMDNResNet50DeepRepViz(nn.Module):  
    def __init__(self, out_classes=1, 
      ...
      #this PMDNResNet only works with this in_shape, if you want to change shape, change shape in PMDN layer
        ...
        block = Bottleneck
        block_inplanes = get_inplanes()
        self.features_3D = nn.Linear(block_inplanes[3] * block.expansion, 3)
        self.relu = nn.ReLU(inplace=True)
        self.classifier = nn.Linear(3, out_classes)
...
class PMDNResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.ln1 = nn.LayerNorm(normalized_shape=[96, 57, 48])
        self.pmdn1 = PMDNLayer(batch_size, num_metadata,  np.prod([96, 57, 48]) * self.in_planes)  
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, block_inplanes[0], layers[0],
                                       shortcut_type)
