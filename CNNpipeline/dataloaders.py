...
class arrayDatasetWeights(arrayDataset):
    def __init__(self, X, y, weights, **kwargs):
        super().__init__(X, y, **kwargs) 
        self.weights = weights
    
    def __getitem__(self, idx):
        sample = super().__getitem__(idx)
        sample.update({'weights': self.weights[idx]})
        return sample
        
class arrayDatasetWithSubIDWeights(arrayDataset):
    def __init__(self, X, y, sub_ids, weights, **kwargs):
        super().__init__(X, y, **kwargs) 
        self.i = sub_ids
        self.weights = weights
        assert len(y) == len(sub_ids)
    ...
    def __getitem__(self, idx):
        sample = super().__getitem__(idx)
        sample.update({'i': int(self.i[idx]), 'weights': self.weights[idx]})
        return sample