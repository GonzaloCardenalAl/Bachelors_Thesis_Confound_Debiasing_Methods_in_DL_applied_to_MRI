...
def cnn_pipeline(
    ...
    task_type='classif',
    weight_debiased = False, cf_name = None,
    label_encoding = False,label_encoding_confound= False,
    n_classes = None,
    **kwargs
): 
    """
...
randomly sampled n={}/{} as validation data".format(len(data_val_idx), len(y_data)))
            ...
            #computing weights for unbalanced classes, all functions used here are available in nitorch.utils
            if weight_debiased == True:
                print("Running weigh debiasing method for "+cf_name+" as a confound")
                if not debug:
                    c_data = data[cf_name][:]
                else:
                    c_data = data[cf_name][:debug_size]
               
                if task_type == 'classif':
                    y_encoded=LabelEncoder()
                    y_data = y_encoded.fit_transform(y_data)
                    print(f'LINE162 DATA ENCODED:{y_data} ')
                    df = pd.DataFrame({'col1' : y_data, 'col2': c_data})
                    counts, categories, samples_num = category_counts(df, 'col1', 'col2')
                    catmultiplication = multiply_categories(df, 'col1', 'col2')
                    weights_dict = divide_dicts(catmultiplication, counts, samples_num)
                    y_data_series = pd.Series(y_data)
                    print(f'MAX Value {y_data_series.max()}')
                    #weights_dict = extend_dict_value(weights_dict, y_data_series.max())
                    #print(f'WEIGHTS DICT {weights_dict}')
                    get_weights(df, 'col1', 'col2', weights_dict)
                    print(f'N_CLASSES{n_classes}')
                    weights=extend_weights(df, 'weight', 'col1',  n_classes)
                    ...
                    weights_dict = divide_dicts(catmultiplication, counts, samples_num)
                    get_weights(df, 'col1', 'col2', weights_dict)
                    weights = pd.Series.to_numpy(df['weight'])
                
                #print(f"CATCOUNTS {counts}")
                #print(f"CATMULTIPLICATION {catmultiplication}")
                weights = torch.from_numpy(weights)
                #criterion_params ={'weight': weights}

                print("Weights for imbalanced data bias were created...")
            ...
            if label_encoding == True:
               # y_encoded=LabelEncoder()
               # y_data = y_encoded(y_data)
                print(f"Y data binned{y_data}")
                if label_encoding_confound == True:
                    c_encoded=LabelEncoder()
                    c_data = c_encoded.fit_transform(c_data)
                    print(f"Confound binned {c_data}")
...
{criterion.__name__} loss function doesn't have any 'weight' argument.")
                        ...
                        if weight_debiased == True:
                            train_data = arrayDatasetWeights(X_data[mask], y_data[mask],
                                                  transform=transform, 
                                                  lbl_type=task_type, weights = weights )
                        else: 
                        ...
                        if weight_debiased == True: 
                            ...
                            trainer = Trainer(model,
                                        ...
                                        device=gpu, task_type=task_type,weight_debiased = True, weights = weights,          
