...
################# Fine-tuning with the parameters loaded from videos ('pretrain' : True, 'feature_extraction' : False (default))
        ...
        'highalc': 
            dict(
                ...
                METRICS=[balanced_accuracy_score, accuracy_score, mz_rsquare],
                MODEL_SETTINGS =[
                    {**common_model_settings,
                    "model_name": "FCN3D_sex", # unique name to identify this model
                     "model_params":{'convs': [16, 32, 64, 96, 128, 256, 96, 64],
                                  'pools': ['', 'max', '', 'max', '', '', 'max', ''],
                                  'kernels': 3,
                                  'dropout': [0.2, 0.2, 0.2, 0.1, 0.1],
                                  'in_shape': (96, 114, 96),
                                  'out_classes':1,
                                  'debug_print': True}, 
