...
class Trainer:
    ...
    def __init__(
        ...
        self.weight_debiased = weight_debiased
    ...
    def arrange_data(
        ...
        try:
            if weights_key is not None:
                if isinstance(data, dict):
                    inputs, labels, weights = data[inputs_key], data[labels_key], data[weights_key]
                    #print("from the arrange method")
        ...
        if weights_key is not None:
            if isinstance(weights, list):
                weights = [wei.to(self.device) for wei in weights]
            else:
        ...
        for epoch in range(num_epochs):
            ...
            if self._stop_training:
                ...
                for i, data in enumerate(train_loader):
                    if self.weight_debiased == True:
                        inputs, labels, weights = self.arrange_data(data, inputs_key, labels_key, weights_key='weights') #from the dataloader. Weights need to be computed there
                        #loss = self.criterion(weight=weights)
                    else:
                   ...
                   # if reweights == True & task_type = 'BinaryClassif':
                    #    loss = self.criterion(outputs, labels, weights = weights)
                    #else: 
                    if self.weight_debiased == True:
                        #print(outputs, outputs.size(), labels, labels.size(), weights, weights.size())
                        classiflist =['classif']
                        if self.task_type == classiflist :
                            loss = 0
                            for i in range(labels.size(dim=0)):
                                weights_i = weights[i].float()
                                criterion_weights_class = self.criterion_weights(weight=weights_i, reduction='none').cuda()
                                loss_one_step = criterion_weights_class(outputs[i], labels[i])
                                loss+=loss_one_step
                            loss=loss/labels.size(dim=0)
                        else:
                            criterion_weights_class = self.criterion_weights(weight=weights, reduction='mean').cuda()
                            print(f'DEBUG 328 {outputs, outputs.size(),labels, labels.size()}')
                            loss = criterion_weights_class(outputs, labels)
                    else: 
                    ...
                    #else:
                    #print(f'DEBUG LOSS 340{loss, loss.item()}')
                ...
                #print(f'DEBUG ALLOUTPUTS{all_outputs[i]}')
                    ...
                    with torch.no_grad():
                        for i, data in enumerate(val_loader):
                            ...
                            if self.training_time_callback is not None:
                                ...
                                outputs = self.model(inputs)
                            print(f'DEBUG LINE 390 {outputs,outputs.shape, labels, labels.shape}')
                            loss = self.criterion(outputs, labels)
    ...
    def _estimate_and_report_metrics(
        ...
        if self.multitask:
            ...
            for metric in self.metrics[task_idx]:
                #fast implementation for computing deviance without tweaking nitorch
                if metric==explained_deviance:
                    result = metric(all_label, y_pred_logits=all_outputs)
                elif metric==mz_rsquare:
                    result = metric(all_outputs)
                else:
